...
@dataclass
class GASPipelineOutput(BaseOutput):
    r"""
    Output class for GAS pipeline.
...
class GASPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        if task == "mv":
            ...
            # original SVD guidance schedule
        ...
        if task == 'mv':
            task_label = torch.tensor([0, 1]).float() # for novel view generation; task label is [0, 1]
        elif task == 'animation':
            task_label = torch.tensor([1, 0]).float() # for novel pose animation; task label is [1, 0]
        else:
            print(f'check task label')
            exit()

        task_label = task_label.reshape(batch_size, -1).to(device)
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    nerf_latents = nerf_latents_all[idx]
        ...
        return GASPipelineOutput(frames=frames)