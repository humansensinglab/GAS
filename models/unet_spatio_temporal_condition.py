...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # task embedding : animation or multiview generation
        self.class_proj = Timesteps(addition_time_embed_dim, True, downscale_freq_shift=0)
        self.class_embedding = TimestepEmbedding(task_embed_input_dim, time_embed_dim)
    ...
    def forward(
            ...
            nerf_latents: torch.Tensor = None,
        ...
        if self.class_embedding is not None:
            # add task embedding according to wonder3d
            if class_labels is None:
                raise ValueError("class_labels should be provided when num_class_embeds > 0")
            class_emb = self.class_proj(class_labels.flatten()) # 16
            class_emb = class_emb.reshape(batch_size, -1)
            class_emb = class_emb.to(emb.dtype) # shape (1, 512)
            # import pdb; pdb.set_trace()
            class_emb = self.class_embedding(class_emb)
            
            emb = emb + class_emb

        time_embeds = self.add_time_proj(added_time_ids.flatten()) # mv 20
        time_embeds = time_embeds.reshape((batch_size, -1))
        time_embeds = time_embeds.to(emb.dtype)
        aug_emb = self.add_embedding(time_embeds) # mv: 22
        emb = emb + aug_emb
        ...
        # add nerf latents
        if nerf_latents is not None:
            sample = sample + nerf_latents
